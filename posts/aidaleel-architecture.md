...
GPT-3.5 is also not fine-tunable through the OpenAI API, but this is fine as for our use case, as a little prompt engineering can already yield great results. In the context of the OpenAI GPT API, [fine-tuning](https://platform.openai.com/docs/guides/fine-tuning/prepare-training-data) simply means providing a pair of prompt and expected response to the model:
```
